...
Modified on 20 Aug 2020 to compute RMSDs of all xyz files in 
the clusters subdirectory and prints results in a csv.
"""
...
def calc_rmsd_value(structure_a, structure_b, args):
    p_all_atoms, p_all = get_coordinates(structure_a, args.format)
    q_all_atoms, q_all = get_coordinates(structure_b, args.format)
...
"""
    ...
    if args.output:
        ...
        # done and done
        # xyz = set_coordinates(q_all_atoms, q_all, title="{} - modified".format(structure_b))
        # print(xyz)
...
"""
    ...
    # format and print
    parser.add_argument('--format', action='store', help='format of input files. valid format are xyz and pdb', metavar='FMT', default = 'xyz')
